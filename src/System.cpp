...
System::System(const string &strVocFile, const string &strSettingsFile, const bool bUseViewer, bool is_load_map_)
    : is_load_map(is_load_map_), mpViewer(static_cast<Viewer *>(NULL)), mbReset(false),
      mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false)
{
    ...
    if (is_load_map && !mapfile.empty() && LoadMap(mapfile))
    {
        bReuseMap = true;
    }
    else
    ...
    mpFrameDrawer = new FrameDrawer(mpMap, nFaceH, nFaceW, nMarginH, nMarginV, bReuseMap);
    mpMapDrawer = new MapDrawer(mpMap, strSettingsFile);

    //Initialize the Tracking thread
    //(it will live in the main thread of execution, the one that called this constructor)
    mpTracker = new Tracking(this, mpVocabulary, mpFrameDrawer, mpMapDrawer,
                             mpMap, mpKeyFrameDatabase, strSettingsFile, bReuseMap);
...
void System::SaveKeyFrameTrajectoryTUM(const string &filename)
{
    ...
    for (size_t i = 0; i < vpKFs.size(); i++)
    {
        ...
        mTimes += pKF->mTimeStamp;
        
        f << setprecision(6) << mTimes << setprecision(7) << " " << t.at<float>(0) << " " << t.at<float>(1) << " " << t.at<float>(2)
...
char System::GetKeyboardInput()
{
    return mpViewer->key;
}

void System::SetViewerFrameCnt(float frameCnt)
{
    mpViewer->frameCnt = frameCnt;
}

void System::SetViewerCurFramePos(float curFramePos)
{
    mpViewer->curFramePos = curFramePos;
}
...
void System::OpenSerialPort()
{
    ...
    /* open serial port */
    fd = open("/dev/ttyACM0", O_RDWR | O_NOCTTY);   //예빈 0208
    cout << "FD: " << fd << endl;

    
    // 시리얼 포트 통신 환경 설정   //예빈 0208
