...
KeyFrame::KeyFrame():
    mnFrameId(0),  mTimeStamp(0.0), mnCubeFaces(CUBEMAP_FACES), mnCubeFaceGridCols(CUBEFACE_GRID_COLS), mnCubeFaceGridRows(CUBEFACE_GRID_ROWS),
    mfGridElementLengthInv(0.0),
    mnTrackReferenceForFrame(0), mnFuseTargetForKF(0), mnBALocalForKF(0), mnBAFixedForKF(0),
    mnLoopQuery(0), mnLoopWords(0), mnRelocQuery(0), mnRelocWords(0), mnBAGlobalForKF(0),
    N(0), mnScaleLevels(0), mfScaleFactor(0),
...
void KeyFrame::serialize(Archive &ar, const unsigned int version)
{
    ...
    // Grid related vars
    ar & const_cast<int &>(mnCubeFaces);
    ar & const_cast<int &>(mnCubeFaceGridCols);
    ar & const_cast<int &>(mnCubeFaceGridRows);
    ...
    // To do : mvKeyRays serializ
    ar & const_cast<std::vector<cv::Vec3f> &>(mvKeyRays);
